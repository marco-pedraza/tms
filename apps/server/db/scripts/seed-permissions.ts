...
async function seedPermissions() {
  try {
    ...
    if (controllerFiles.length === 0) {
      ...
      console.log('Please check that the script is running from the correct directory');
    }
    ...
    for (const relativeFilePath of controllerFiles) {
      ...
      const serviceName = serverIndex !== -1 && serverIndex + 1 < serviceSegments.length
      ...
      const endpoints = extractApiEndpoints(fileContent, serviceName, featureName);
    ...
    for (const permission of uniquePermissions) {
      try {
        ...
        try {
          await permissionRepository.findByCode(permission.code);
      ...
      if (apiConfig && ts.isObjectLiteralExpression(apiConfig)) {
        ...
        for (const property of apiConfig.properties) {
          if (ts.isPropertyAssignment(property)) {
            if (property.name.getText() === 'method' && ts.isStringLiteral(property.initializer)) {
              method = property.initializer.text;
            } else if (property.name.getText() === 'path' && ts.isStringLiteral(property.initializer)) {
