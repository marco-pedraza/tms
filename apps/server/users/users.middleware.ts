import { APIError, middleware, MiddlewareRequest, APICallMeta } from 'encore.dev/api';
...
export const usersMiddleware = middleware(
    ...
    async (req: MiddlewareRequest, next) => {
      ...
      const user = await userPermissionsRepository.getUserWithPermissions(auth.userID);
