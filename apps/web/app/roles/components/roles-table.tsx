...
import useQueryRoles from '@/roles/hooks/use-query-roles';
import useRoleMutations from '@/roles/hooks/use-role-mutations';
import routes from '@/services/routes';
import type {
  UseCommonTranslationsResult,
  UseRolesTranslationsResult,
} from '@/types/translations';

interface RolesColumnsFactoryProps {
  tCommon: UseCommonTranslationsResult;
  tRoles: UseRolesTranslationsResult;
}

function rolesColumnsFactory({
  tCommon,
}: RolesColumnsFactoryProps): DataTableColumnDef<roles.Role>[] {
  return [
    {
      accessorKey: 'name',
...
export default function RolesTable() {
  ...
  const {
    ...
    filtersUrlState,
    setFiltersUrlState,
  } = useTableUrlState<roles.Role>();
  const { data, isLoading, error, refetch } = useQueryRoles({
  ...
  const { deleteId, setDeleteId, onConfirmDelete, onCancelDelete } =
    useDeleteDialog({
      onConfirm: deleteRole.mutateWithToast,
    });

  const columns = rolesColumnsFactory({
    tCommon,
    tRoles,
  });
